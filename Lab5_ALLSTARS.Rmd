...
milk_avg <- milk %>%
  group_by(year) %>%
  summarize(avg_milk=mean(milk_millions))
ggplot() +
  geom_point(milk_hawaii, mapping=aes(x=year, y=milk_millions)) +
...
```{r}
mean <- milk_1999 %>%
  summarize(Mean=mean(milk_millions))

median <- milk_1999 %>%
  summarize(Median=median(milk_millions))

most_milk <- milk_1999 %>%
  filter(rank(desc(milk_millions))<2)

least_milk <- milk_1999 %>%
  filter(rank(milk_millions)<2)
